...
export default function TimelineCard({ eventos }: TimelineCardProps) {
  ...
  return (
    <div className="aurora-card p-6 md:p-8 flex flex-col justify-between transition-smooth hover:translate-y-[-2px] hover:shadow-xl bg-white border border-slate-100">
      <div>
        ...
        {safeEventos.length > 0 ? (
          <div className="space-y-4 mb-6">
            {safeEventos.slice(0, 4).map((evento) => (
              <div key={evento.id} className="flex gap-4 p-3 rounded-2xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                <div className="mt-1">
                  <div className="w-2 h-2 rounded-full bg-blue-500 mt-1.5 shadow-[0_0_8px_rgba(59,130,246,0.5)]"></div>
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-slate-900 truncate">{evento.titulo}</h3>
          ...
          <div className="py-8 text-center bg-slate-50 rounded-2xl mb-6 border border-slate-100 border-dashed">
            <Calendar size={24} className="mx-auto text-slate-300 mb-2" />
            <p className="text-sm text-slate-500">Sua agenda está livre!</p>
      ...
      <Link to="/integracoes" className="group flex items-center justify-between w-full py-4 px-6 bg-slate-50 text-slate-700 rounded-2xl hover:bg-blue-50 hover:text-blue-700 transition-smooth font-medium active:scale-95 border border-slate-100">
